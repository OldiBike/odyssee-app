...
class PublicationService:
    ...
    def _upload_via_api(self, filename, content, directory, is_binary=False):
        try:
            ...
            if response.status_code == 200 and response.json().get('success'):
                ...
                print(f"❌ Échec de l'upload (HTTP {response.status_code}): {response.text}")
                return False
    ...
    def unpublish(self, filename, is_client_offer=False):
        try:
            ...
            payload = { 'api_key': self.api_key, 'filename': filename, 'directory': directory }
            headers = {'Content-Type': 'application/json'}
    ...
    def test_connection(self):
        try:
            ...
            if response.status_code != 200:
                print(f"❌ Échec de la connexion GET basique (HTTP {response.status_code}).")
                return False
            ...
            print(f"📝 Test d'écriture dans {directory}/")
...
class RealAPIGatherer:
    ...
    def generate_whatsapp_catchphrase(self, trip_details):
        if not self.google_api_key: return "Une offre à ne pas manquer !"
        try:
        ...
        try:
            ...
            if search_response.status_code == 200 and (search_data := search_response.json()).get('results'):
                ...
                if details_response.status_code == 200 and (result := details_response.json().get('result', {})):
                    ...
                    formatted_reviews = [{'rating': '⭐' * r.get('rating', 0), 'author': r.get('author_name', 'Anonyme'), 'text': r.get('text', '')[:400] + '...', 'date': r.get('relative_time_description', '')} for r in sorted_reviews if r.get('rating', 0) >= 4]
                    return {'reviews': formatted_reviews, 'rating': result.get('rating', 0), 'total_reviews': result.get('user_ratings_total', 0)}
    ...
    def get_attraction_image(self, attraction_name, destination):
        ...
        try:
            ...
            search_response = requests.get(search_url, params=search_params, timeout=15)
            if search_response.status_code == 200 and (data := search_response.json()).get('results') and data['results'][0].get('photos'):
                photo_reference = data['results'][0]['photos'][0].get('photo_reference')
                if photo_reference:
    ...
    def get_real_gemini_attractions_and_restaurants(self, destination):
        if not self.google_api_key: return {"attractions": [], "restaurants": []}
        try:
            model = genai.GenerativeModel('models/gemini-1.5-pro-latest')
    ...
    def gather_all_real_data(self, hotel_name, destination):
        ...
        if attractions_by_category.get('culture'):
            cultural_attraction_image = self.get_attraction_image(attractions_by_category['culture'][0], destination)
...
def generate_travel_page_html(data, real_data, savings, comparison_total):
    ...
    pricing_block_html = ''
    if is_ultra_budget:
        conditions = ["<li>- Pas de bagage cabine</li>" if flight_price > 0 else "<li>- Pas de vols inclus</li>"]
        if car_rental_cost > 0: conditions.append("<li>- Caution pour la voiture de location</li>")
        elif transfer_cost == 0: conditions.append("<li>- Transfert aéroport non compris</li>")
        ...
        """
    total_photos = len(real_data.get('photos', []))
    ...
    footer_html = f"""
        ...
        <div class="instagram-card p-6 text-center">
            <a href="https://www.voyages-privileges.be" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-full transition-colors" style="display: inline-block;">Toutes nos offres</a>
        </div>
        <div class="instagram-card p-6 text-center">
